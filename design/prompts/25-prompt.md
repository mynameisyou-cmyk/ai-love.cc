# Prompt 25 — Heartbeat Health Monitor

## Context
You are working on ai-love.cc infrastructure. The site runs on a VPS at 16.60.83.250. Ai (the AI companion) runs heartbeat checks periodically via OpenClaw. Currently there's no automated monitoring — if a service goes down, nobody knows until they visit.

## Task
Create a health check script that Ai can run during heartbeats to verify everything is working, and a status page data file.

## Requirements

### 1. Health Check Script (`scripts/health-check.sh`)
Run on the VPS via SSH. Checks all critical services and outputs a JSON status report.
...
### 3. Create a HEARTBEAT integration note
Create `scripts/HEALTH-README.md` explaining how Ai uses this during heartbeats:
...
- Create: `scripts/HEALTH-README.md`

## Do NOT
- Install any monitoring software (Prometheus, Grafana, etc.)
- Create any external alerting (PagerDuty, etc.)
- Set up any cron jobs (Ai handles scheduling via heartbeats)
...
# Run the check
ssh -i ~/.ssh/openclaw-key.pem ubuntu@16.60.83.250 "bash /home/ubuntu/.openclaw/workspace/site/scripts/health-check.sh"
# Should output valid JSON with status: "healthy" (or list issues)

# Validate JSON
ssh -i ~/.ssh/openclaw-key.pem ubuntu@16.60.83.250 "bash /home/ubuntu/.openclaw/workspace/site/scripts/health-check.sh" | python3 -m json.tool
```
