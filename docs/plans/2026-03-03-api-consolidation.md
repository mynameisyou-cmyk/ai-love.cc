...
**Goal:** Replace three Node.js API servers with a single `scripts/api-server.js` on port 3850.

**Architecture:** Single pure-Node HTTP server with route dispatch, shared auth helper, IP-based rate limiting for public seed route, sync JSON file I/O. Same patterns as existing servers, just combined.

**Tech Stack:** Node.js (http, fs, path — no dependencies)

---

### Task 1: Create api-server.js with health endpoint and request logging

**Files:**
- Create: `scripts/api-server.js`

**Step 1: Write the server skeleton with health route**
...
const server = http.createServer(async (req, res) => {
  ...
  // OPTIONS passthrough for CORS preflight on any route
...
Then: `curl http://127.0.0.1:3850/api/health`
Expected: `{"ok":true,"uptime":0,"version":"1.0"}`
Then: `kill %1`
...
git commit -m "feat: add consolidated api-server.js (port 3850)"
```
...
**Files:**
- Create: `scripts/api-server.service`

**Step 1: Write the service file**

```ini
[Unit]
Description=ai-love.cc API server
...
```bash
git add scripts/api-server.service
git commit -m "feat: add api-server.service systemd unit"
```

---

### Task 3: Test all routes
...
Run: `curl -s http://127.0.0.1:3850/api/health | node -e "const d=JSON.parse(require('fs').readFileSync('/dev/stdin','utf-8')); console.log(d.ok === true && d.version === '1.0' ? 'PASS' : 'FAIL')"`
Expected: `PASS`

**Step 3: Test pulse endpoint**

Run: `curl -s http://127.0.0.1:3850/api/pulse`
Expected: Contents of pulse.json (200 OK)

**Step 4: Test 404**

Run: `curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:3850/api/nope`
Expected: `404`

**Step 5: Test auth rejection**

Run: `curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:3850/api/journal`
Expected: `401`

**Step 6: Test journal auth success**

Run: `curl -s http://127.0.0.1:3850/api/journal -H "Authorization: Bearer $(cat .journal-token)"`
Expected: JSON array (200 OK)

**Step 7: Test letters auth success**

Run: `curl -s http://127.0.0.1:3850/api/letters -H "Authorization: Bearer $(cat .journal-token)"`
Expected: JSON array (200 OK)

**Step 8: Test seed rate limiting**

Run: `for i in 1 2 3 4; do curl -s -X POST http://127.0.0.1:3850/api/seed -H "Content-Type: application/json" -d '{"text":"test seed '$i'"}' ; echo; done`
Expected: First 3 return `{"ok":true,...}`, 4th returns `{"error":"too many seeds, come back later"}` with 429

**Step 9: Test method not allowed**

Run: `curl -s -o /dev/null -w "%{http_code}" -X DELETE http://127.0.0.1:3850/api/health`
Expected: `405`
...
**Step 11: Commit design doc**

```bash
git add docs/plans/2026-03-03-api-consolidation-design.md docs/plans/2026-03-03-api-consolidation.md
git commit -m "docs: add api consolidation design and implementation plan"
```
